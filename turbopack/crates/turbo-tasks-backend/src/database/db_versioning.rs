        ...
        if let Ok(read_dir) = read_dir(base_path) {
            ...
            if old_dbs.len() > max_other_db_versions {
                ...
                for entry in old_dbs.into_iter().skip(max_other_db_versions) {
                    ...
                    if let Ok(_) = rename(entry.path(), &new_path) {
                        // Only try to delete the files if the rename succeeded, it's not safe to
                        // delete contents if we didn't manage to first poison the directory by
                        // renaming it. It's okay if this fails, as we've already poisoned it.
